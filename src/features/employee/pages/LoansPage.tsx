...
export function EmployeeLoansPage() {
  ...
  const { data: loans, isLoading } = useQuery({
    queryKey: ['employee-loans', employee?.id, statusFilter],
    queryFn: async () => {
      if (!employee?.id || !profile?.agency_id) return [];

      let query = supabase
        .from('loans')
        .select('*, customers(customer_id, users(full_name, email))')
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-3 gap-4 text-sm">
                        ...
                        <div>
                          ...
                          <p className="text-slate-600">{formatDateSafe(loan.created_at)}</p>
                        </div>
                      </div>
                    </div>
